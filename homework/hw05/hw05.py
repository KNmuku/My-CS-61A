...
def store_digits(n):
    ...
    def split_list(n):
        if not n:
            return []
        return split_list(n//10) + [n%10]
    
    def generate_link(lst):
        if not lst:
            return Link.empty
        else:
            return Link(lst[0], generate_link(lst[1:]))
        
    return generate_link(split_list(n))
...
def is_bst(t):
    ...
    if len(brs) > 2:
        ...
        if not is_bst(brs[0]) or not is_bst(brs[1]):
            return False
        return bst_max(brs[0]) <= t.label < bst_min(brs[1])
    else:
        if not is_bst(brs[0]):
            return False
        return bst_max(brs[0]) <= t.label or t.label < bst_min(brs[0]) 
...
def preorder(t):
    ...
    """
    "*** YOUR CODE HERE ***"
    return sum([[t.label]] + [preorder(b) for b in t.branches], [])
