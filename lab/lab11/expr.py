...
class CallExpr(Expr):
    ...
    def eval(self, env):
        ...
        result = self.operator.eval(env).apply(args)
        return result

        # Alternative Solution
        args = [arg.eval(env).values for arg in self.operands]
        result = self.operator.eval(env).operator(*args)
...
class LambdaFunction(Value):
    ...
    def apply(self, arguments):
        ...
        "*** YOUR CODE HERE ***"
        new_frame = self.parent.copy()
        new_frame.update({para: argu for para, argu in zip(self.parameters, arguments)})
        return self.body.eval(new_frame)
