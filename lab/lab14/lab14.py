...
def num_splits(s, d):
    ...
    """
    "*** YOUR CODE HERE ***"
    def split_helper(s, difference):
        if not s:
            if abs(difference) <= d:
                return 1
            return 0
        return split_helper(s[1:], difference+s[0]) + split_helper(s[1:], difference-s[0])
    return split_helper(s, 0) // 2
...
class Check(object):
    def __init__(self, check_holder, check_amount):
...
def align_skeleton(skeleton, code):
    ...
    def helper_align(skeleton_idx, code_idx):
        ...
        if skel_char == code_char:
            ...
            print("DEBUG:", match_rest_result, match_rest_cost)
...
def filterl(lst, pred):
    ...
    "*** YOUR CODE HERE ***"
    z = Link.empty 
    fn = lambda x, z: Link(x, z) if pred(x) else z
    return foldr(lst, fn, z)
    

# alternative solution: filter_right  
""" 
...
def reverse(lst):
    ...
    """
    "*** YOUR CODE HERE ***"
    return foldl(lst, lambda z, x: Link(x, z), Link.empty)
