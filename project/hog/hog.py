...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0 < goal and score1 < goal:
        if not who:
            ...
            who = other(who) if not extra_turn(score0, score1) else who
        else:
            score1 += take_turn(strategy1(score1, score0), score0, dice)
            who = other(who) if not extra_turn(score1, score0) else who
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(score0, score1):
        ...
        if not who:
            ...
            curr = score1
        if (curr - last) > highest:
            print(curr - last, 'point(s)! The most yet for Player', who)
            highest = curr - last
        last = curr
        return announce_highest(who, last, highest)
    return say
    # END PROBLEM 7
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    i, max_i, max_aver = 1, 0, 0
    aver_score = make_averaged(roll_dice, trials_count)
    while i <= 10:
        aver = aver_score(i, dice)
        if aver > max_aver:
            max_aver = aver
            max_i = i
        i += 1
