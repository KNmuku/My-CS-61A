...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        if isinstance(fn, MacroProcedure):
            return scheme_eval(fn.apply_macro(rest, env), env)
        rest = rest.map(lambda x: scheme_eval(x, env)) # don't mutate
        return scheme_apply(fn, rest, env) 
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        name = target.first
        args = target.rest
        lambda_body = expressions.rest
        lambda_inst = do_lambda_form(Pair(args, lambda_body), env)
        env.define(name, lambda_inst)
        return name
        # END PROBLEM 9
    else:
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        name_value_pair = bindings.first
        validate_form(name_value_pair, 2, 2)
        name, value = name_value_pair.first, scheme_eval(name_value_pair.rest.first, env)
...
def do_define_macro(expressions, env):
    ...
    try:
        ...
        if not scheme_symbolp(name):
            raise SchemeError('macro has no name')
        formals = expressions.first.rest
        body = expressions.rest
        if body is nil:
            raise SchemeError('macro has no body')
        macro_inst = MacroProcedure(formals, body, env)
        env.define(name, macro_inst)
        return name
    except AttributeError:
        raise SchemeError('can only define-macro procedures')
    # END Problem 20
