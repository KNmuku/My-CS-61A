...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        distance = 0
        with_range = lambda x: self.lower_bound <= x <= self.upper_bound
        while not target_place.is_hive:
            if with_range(distance):
                if target_place.bees:
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        # BEGIN Problem 5
        "*** YOUR CODE HERE ***"
        remain_health = self.health - amount
...
# The HungryAnt Class
class HungryAnt(Ant):
    """HungryAnt eat a random bee from its place and 
    spend 3 turns chewing it."""
...
class TankAnt(ContainerAnt):
    """TankAnt protects an ant in its place and deals 1 damage to
    all bees in its place eace turn."""
...
# END Problem 12
    ...
    def action(self, gamestate):
        ...
        # BEGIN Problem 12
        "*** YOUR CODE HERE ***"
        target_place = self.place.exit
        while target_place:
            if target_place.ant:
                if target_place.ant.is_container and target_place.ant.ant_contained:
                    target_place.ant.ant_contained.double()
                target_place.ant.double()
            target_place = target_place.exit
        super().action(gamestate)
        # END Problem 12
...
class SlowThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        # BEGIN Problem EC 1
        "*** YOUR CODE HERE ***"
        def new_action(target_bee):
            def slow_helper(gamestate):
                if target_bee.turns_to_slow:
                    if not gamestate.time % 2:
                        Bee.action(target_bee, gamestate)  
                    target_bee.turns_to_slow -= 1
                else:
                    Bee.action(target_bee, gamestate)
            return slow_helper
        target.turns_to_slow = self.slow_turns
        target.action = new_action(target)
            # END Problem EC 1
...
class ScaryThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        # BEGIN Problem EC 2
        "*** YOUR CODE HERE ***"
        target.turns_scared = self.scare_turns
        # END Problem EC 2
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        elif self.turns_scared and not self.has_been_scared:
            self.scare()
    ...
    def scare(self):
        ...
        if self.turns_scared and not self.has_been_scared:
            if not back.is_hive:
                self.move_to(back)
            self.turns_scared -= 1
            if not self.turns_scared:
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self):
        ...
        distance = 0
        insects = {}
        while not front.is_hive:
            if front.ant and front.ant is not self:
                if front.ant.is_container:
                    if front.ant.ant_contained is not self:
                        insects[front.ant.ant_contained] = distance
                insects[front.ant] = distance
            elif front.bees:
                for bee in front.bees:
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem Optional 2
        return max(0, self.damage - self.insects_shot * 0.0625 - distance * 0.25)
        # END Problem Optional 2
